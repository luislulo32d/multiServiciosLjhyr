$(document).ready(function(){
    ...
    $('.btn-changueD').on('click', function(){
    	swal({
		  	title: '¿Desea Cambiar el Precio del Dolar?',
			 html: `<input type="number" id="precioDolar1" class="swal2-input" placeholder="25.5"  step="0.01" maxlength="3" required>`,
		  	type: 'info',
		...
		function(isConfirm) {
		  	if (isConfirm) {
				var precioDolar = document.getElementById("precioDolar1").value;
				if (precioDolar != "") {
					console.log(precioDolar);
					swal("¡Completado!","Actualizado Correctamente a: "+ precioDolar, "success");
				}else{
					swal("¡Error!","¡Debe ingresar un valor para guardarlo!", "error");
				}
